...
import type { MonadIO4 } from 'fp-ts/MonadIO'
import type { MonadTask4 } from 'fp-ts/MonadTask'
import type { MonadThrow4 } from 'fp-ts/MonadThrow'
import type { Option } from 'fp-ts/Option'
import { flow, identity, Predicate, Refinement } from 'fp-ts/function'
import { pipe } from 'fp-ts/function'
import type { MonadObservable4 } from './MonadObservable'
 ...
 */
export const chainW =
    <A, S, R2, E2, B>(f: (a: A) => StateReaderObservableEither<S, R2, E2, B>) =>
    <R1, E1>(ma: StateReaderObservableEither<S, R1, E1, A>): StateReaderObservableEither<S, R1 & R2, E1 | E2, B> =>
    s1 =>
        pipe(
            ma(s1),
...
export const fromPredicate: {
    ...
    <S, R>(a: A): StateReaderObservableEither<S, R, E, A> =>
...
) => StateReaderObservableEither<S, R1 & R2, E1 | E2, { [P in keyof A | K]: P extends keyof A ? A[P] : B }> =
    bind as any
